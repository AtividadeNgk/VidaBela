...
async def is_admin(context, user_id, show_plans_if_not_admin=True):
    if (str(user_id) in manager.get_bot_admin(context.bot_data['id']) or is_owner(context, user_id)):
        ...
        if show_plans_if_not_admin:
            ...
            await context.bot.send_message(user_id, 'Escolha uma das oferta abaixo:　', reply_markup=reply_markup)
        return False
...
async def process_command(update: Update, context):
    # Pega o nome do comando atual (sem a /)
    current_command = update.message.text.split()[0][1:] if update.message.text else ""
    
    conv_state = context.user_data.get('conv_state', False)
    
    if conv_state != False:
        # NOVO: Verifica se é o MESMO comando sendo executado novamente
        if conv_state == current_command:
            keyboard = InlineKeyboardMarkup([[
                InlineKeyboardButton('❌ Cancelar comando', callback_data='cancelar')
            ]])
            
            await update.message.reply_text(
                f'⚠️ O comando /{conv_state} já está em execução\\!\n\n'
                f'Você estava no meio da configuração\\.\n'
                f'Cancele para recomeçar\\.',
                reply_markup=keyboard,
                parse_mode='MarkdownV2'
            )
            return False
        
        # Comando diferente (mantém comportamento original)
        else:
            keyboard = InlineKeyboardMarkup([[InlineKeyboardButton('❌ Cancelar', callback_data='cancelar')]])
            await update.message.reply_text(
                f'⚠️ O comando *{conv_state}* está em execução no momento\n\n'
                f'> Caso deseje cancelar, clique no botão abaixo',
                reply_markup=keyboard,
                parse_mode='MarkdownV2'
            )
            return False
