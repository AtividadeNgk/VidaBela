...
async def send_recovery(context, user_id, recovery_data, recovery_index, bot_id):
    ...
    try:
        ...
        for plan_index in range(len(planos)):
            ...
            # MUDANÇA AQUI: Novo callback que identifica recuperação e índice do plano
            keyboard_plans.append([InlineKeyboardButton(
                botao_texto, 
                callback_data=f"recovery_{plan_index}_{payment_id}"
            )])
